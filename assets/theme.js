...
(function () {
  ...
  theme.pageTransitions = function () {
    if (document.body.dataset.transitions === "true") {
      ...
      document
        .querySelectorAll(
          'a.hero__slide-link, a[href^="mailto:"], a[href^="#"], a[target="_blank"], a[href*="youtube.com/watch"], a[href*="youtu.be/"], a[download]'
        )
        .forEach((el) => {
...
$(document).ready(function () {
  if ($(window).width() > 768) {
  $(document).on("click", ".size-variants .size-swatch", function () {
    if($(this).hasClass('not-available')) return;
    let CsTHis = $(this);
    CsTHis.parent().parent().addClass('product-adding')
    let Thistitle = $(this).text();
    let variantJson = $(this)
      .closest(".grid-product")
      .find(".variantjsonInfo")
      .text();
    variantJson = JSON.parse(variantJson);
    let opIndex = $(this).attr("data-index");
    let variantTitle = "";
    if (
      $(this)
        .closest(".grid-product")
        .find(".grid-product__colors")
    ...
    let filteredArray = variantJson.filter((data) => {
      if (data["option" + opIndex] == Thistitle) {
        return true;
      }
    });
    let finalVariant = filteredArray.filter((data) => {
      if (data["title"].includes(variantTitle)) {
        return true;
      }
    });
    let productData = {
      id: finalVariant[0].id,
      quantity: 1,
    };
    $.ajax({
      type: "POST",
      ...
      success: function (product) {
        ...
        setTimeout(function(){
         CsTHis.parent().parent().removeClass('product-adding'); 
        },2000)
      ...
      error:function(){
       setTimeout(function(){
        ...
        console.log('product not adding')
      }
    });
  }); 
}
    ...
    if($(this).hasClass('not-available')) return;
    let CsTHis = $(this);
       CsTHis.parent().parent().next().removeClass('hide');
    let Thistitle = $(this).text();
    let variantJson = $(this)
      .closest(".grid-product")
      .find(".variantjsonInfo")
    ...
    let finalVariant = filteredArray.filter((data) => {
      if (data["title"].includes(variantTitle)) {
        ...
        CsTHis.parent().parent().next().find('a').attr('data-variant-id',finalVariant[0].id);
  
  });
}
  $('.addtocartGrid a').on('click',function(e){
      e.preventDefault();
      let  CsTHis = $(this);
     CsTHis.closest('.grid-product').find('.size-variants').addClass('product-adding'); 
     let productData = {
      id: parseInt($(this).attr('data-variant-id')),
      quantity: 1,
    };
      $.ajax({
      type: "POST",
      ...
      success: function (product) {
        ...
        CsTHis.parent().addClass('hide');
        setTimeout(function(){
         CsTHis.closest('.grid-product').find('.size-variants').removeClass('product-adding'); 
        },2000)
  ...
  $('.grid-product__colors .color-swatch').on('touchstart mouseenter',function(){
    ...
    variantJson = JSON.parse(variantJson);
    let filteredArray = variantJson.filter((data) => {
      if (data["title"].includes(colorTitle)) {
        return true;
      }
    });
   
   $(this).closest('.grid-product').find('.size-swatch').addClass('not-available');
    for (let index = 0; index < filteredArray.length; index++) {
      if(filteredArray[index].available){
        $(this).closest('.grid-product').find('.size-swatch[data-title="'+filteredArray[index].option1+'"]').removeClass('not-available');
      }
    }
    
  });

  if($('.product-grid .grid-product').length) { 
  $('.product-grid .grid-product').each(function(){
   let colorTitle = $(this).find('.sw-1:first-child').text().trim();
    if(colorTitle){
     let variantJson = $(this)
      .find(".variantjsonInfo")
      .text();
    ...
    for (let index = 0; index < filteredArray.length; index++) {
      ...
      if(filteredArray[index].available){
        $(this).find('.size-swatch[data-title="'+filteredArray[index].option1+'"]').removeClass('not-available');
      }
    }
  }
  });
}
  $('.color-swatch').on('touchstart mouseenter',function(){
    if(!$(this).hasClass('color-swatch--with-image')){
      $(this).closest('.grid-product').find('.color-swatch').removeClass('is-active');
